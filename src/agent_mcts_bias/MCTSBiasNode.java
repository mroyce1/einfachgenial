...
public class MCTSBiasNode implements Comparable<MCTSBiasNode> {
    private static double explorationWeight = 1.44;
    private GameState state;
    private MCTSBiasNode father;
    private ArrayList<HexMove> moves;
    private ArrayList<MCTSBiasNode> children;
    private double uct;
    ...
    public MCTSBiasNode(MCTSBiasNode f, GameState s, HexMove m, boolean first) {
        this.father = f;
        this.state = s;
        ...
        this.children = new ArrayList<MCTSBiasNode>();
        this.uct = 0.0;
        this.visits = 0.0;
        this.wins = 0.0;
        this.won = 0.0;
        this.move = m;
        this.first = first;
        this.id = s.getInactivePlayer().getId();
    }

    @Override
    public String toString() {
        return "uct: " + this.uct + "   visits: " + this.visits + "   wins: " + this.wins + "   bias: " + this.bias +
                "   player: " + this
                .state.getByID(this.id);
    }


    public List<MCTSBiasNode> getChildren() {
        return children;
    }
    ...
    @Override
    public int compareTo(MCTSBiasNode other) {
    ...
    public void setChildren(ArrayList<MCTSBiasNode> children) {
    ...
    private void initializeChild(HexMove m) {
        ...
        m.execute(copy);
        MCTSBiasNode child = new MCTSBiasNode(this, copy, m, this.first);
        child.visits += 1.0;
    ...
    private double runRandomSimulation() {
        ...
        this.bias = calcBias(this.state);
        return calcResult(copy);
    }

    private double calcBias(GameState copy) {
        int sum = Auxiliary.getByteSum(copy.getByID(this.id).getScores());
        if (copy.getMovesPlayed() > 21) {
            return sum;
        }
        return sum * 4;
    }
    ...
    private void propagate(int id, double val) {
        ...
        while (fath != null) {
            fath.wins += this.state.getActivePlayer().getId() == fath.state.getActivePlayer().getId() ? this.won
                    : (1.0 - this.won);
            for (MCTSBiasNode c : fath.children) {
                c.uct = c.wins / c.visits + weight * Math.sqrt(Math.log(fath.visits) / c
                        .visits) + (c.bias / c.visits);
            }
            fath = fath.father;
        }
    }
